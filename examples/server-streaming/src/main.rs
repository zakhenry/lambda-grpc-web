...
pub mod hello_world {
    tonic::include_proto!("helloworld_stream");
}
...
// run with `cargo lambda watch -p example-server-streaming`
// build for aws with `cargo lambda build -p example-server-streaming --release --output-format zip --arm64`
#[tokio::main]
...
// integration, run `cargo lambda watch -p example-server-streaming` first to have the local lambda
